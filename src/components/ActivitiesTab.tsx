...
import { Plus, Trash2, X, Filter, Copy, Edit2 } from "lucide-react";
...
function JournalView({ activities }: { activities: any[] }) {
  ...
  const handleClone = (a: any, e: MouseEvent) => {
    e.stopPropagation();
    const cloned = { ...a };
    delete cloned.id;
    delete cloned._isNew;
    cloned["Дата проведения"] = "";
    setSelectedActivity({
      ...cloned,
  ...
  const handleDelete = async (a: any, e: MouseEvent) => {
    e.stopPropagation();
    if (confirm("Удалить активность?")) {
      await deleteRecord("activities", a.id, a);
    }
  };
  ...
  const sortedData = useMemo(
    () => sortData(normalizedFiltered),
    [normalizedFiltered, sortData],
  );
  ...
  const periods = Array.from(
    new Set(activities.map((a) => a["Период"]).filter(Boolean)),
  )
    .sort()
    .reverse();
  const mentors = Array.from(
    new Set(activities.map((a) => a["Ментор"]).filter(Boolean)),
  ).sort();
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50/50">
          ...
          <div className="flex gap-2">
            <button
              onClick={resetWidths}
              className="text-gray-500 hover:text-gray-700 text-xs px-2.5 py-1.5 border border-gray-300 rounded hover:bg-gray-50 transition-colors"
              title="Сбросить ширину колонок"
            >
              Сбросить ширину
            </button>
            <button
              onClick={() => setSelectedActivity({ _isNew: true })}
            ...
            <thead>
              <tr className="border-b-2 border-gray-200">
                <th
                  ...
                  className="py-2 px-2 cursor-pointer hover:bg-gray-100 select-none bg-white z-10 relative group"
                >
                  <div
                    onClick={() => handleSort("Период")}
                    className="w-full h-full pr-4"
                  >
                    {renderSortIcon("Период")}Период
                  </div>
                  <div
                    onMouseDown={(e) => handleResizeStart(e, "period")}
                    onClick={(e) => e.stopPropagation()}
                    className="absolute right-0 top-0 bottom-0 w-1.5 bg-transparent hover:bg-blue-400 active:bg-blue-600 cursor-col-resize z-20"
                  />
                </th>
                <th
                  style={{
                ...
                >
                  <div
                    onClick={() => handleSort("Дата проведения")}
                    className="w-full h-full pr-4"
                  >
                    {renderSortIcon("Дата проведения")}Дата
                  </div>
                  <div
                ...
                >
                  <div
                    onClick={() => handleSort("Тип активности")}
                    className="w-full h-full pr-4"
                  >
                    {renderSortIcon("Тип активности")}Тип активности
                  </div>
                  <div
                ...
                >
                  <div
                    onClick={() => handleSort("Ментор")}
                    className="w-full h-full pr-4"
                  >
                    {renderSortIcon("Ментор")}Ментор
                  </div>
                  <div
                ...
                >
                  <div
                    onClick={() => handleSort("Программа")}
                    className="w-full h-full pr-4"
                  >
                    {renderSortIcon("Программа")}Программа/Поток
                  </div>
                  <div
                    onMouseDown={(e) => handleResizeStart(e, "program")}
                    onClick={(e) => e.stopPropagation()}
                    className="absolute right-0 top-0 bottom-0 w-1.5 bg-transparent hover:bg-blue-400 active:bg-blue-600 cursor-col-resize z-20"
                  />
                </th>
                <th
                  ...
                  className="py-2 px-2 text-right cursor-pointer hover:bg-gray-100 select-none bg-white z-10 relative group"
                >
                  <div
                    onClick={() => handleSort("Кол-во")}
                    className="w-full h-full pl-4"
                  >
                    {renderSortIcon("Кол-во")}Кол-во
                  </div>
                  <div
                ...
                <th
                  ...
                  className="py-2 px-2 text-center bg-white z-10 relative group"
                >
                  <div className="w-full h-full text-gray-500 font-medium">Действия</div>
                  <div
            ...
            <tbody>
              {paginatedData.map((a) => {
                ...
                return (
                  ...
                  >
                    <td
                      className="py-1.5 px-2 text-gray-500 truncate"
                      style={{
                        width: widths.period,
                        maxWidth: widths.period,
                        overflow: "hidden",
                    ...
                    <td
                      ...
                      style={{
                        width: widths.date,
                        maxWidth: widths.date,
                        overflow: "hidden",
                        textOverflow: "ellipsis",
                        whiteSpace: "nowrap",
                      }}
                      title={a["Дата проведения"]}
                    >
                      {a["Дата проведения"]}
                    </td>
                    <td
                      ...
                      title={a["Тип активности"]}
                    >
                      {a["Тип активности"]}
                    </td>
                    <td
                      className="py-1.5 px-2 font-medium truncate"
                      style={{
                        width: widths.mentor,
                        maxWidth: widths.mentor,
                        overflow: "hidden",
                        textOverflow: "ellipsis",
                        whiteSpace: "nowrap",
                      }}
                    ...
                    <td
                      className="py-1.5 px-2 text-gray-600 truncate"
                      style={{
                        width: widths.program,
                        maxWidth: widths.program,
                        overflow: "hidden",
                        textOverflow: "ellipsis",
                        whiteSpace: "nowrap",
                      }}
                    ...
                    <td
                      className="py-1.5 px-2 text-right truncate"
                      style={{
                        width: widths.qty,
                        maxWidth: widths.qty,
                        overflow: "hidden",
...
function MentorsSummary({ activities }: { activities: any[] }) {
  ...
  const { types, matrix } = useMemo(() => {
    const data = period
      ? activities.filter((a) => a["Период"] === period)
      : activities;
      ...
      <div className="flex-1 overflow-auto border border-gray-200 rounded">
        <table className="w-full text-left border-collapse text-sm">
          <thead className="bg-gray-50 sticky top-0 shadow-sm">
            <tr>
              <th className="py-2 px-3 border-r border-gray-200 w-48">
                Ментор
              </th>
              {types.map((t) => (
                <th
                  key={t}
                  className="py-2 px-3 text-center border-r border-gray-200 whitespace-nowrap"
                >
                  {t}
                </th>
              ))}
              <th className="py-2 px-3 text-center bg-gray-100 font-bold">
                Итог
              </th>
            </tr>
          </thead>
              ...
              return (
                ...
                >
                  ...
                  {types.map((t) => (
                    <td
                      key={t}
                      className={`py-2 px-3 text-center border-r border-gray-200 ${matrix[m][t] ? "" : "text-gray-300"}`}
                    >
                      {matrix[m][t] || "—"}
                    </td>
                  ))}
                  <td className="py-2 px-3 text-center bg-gray-50 font-bold">
                    {rowTotal}
                  </td>
                </tr>
              );
              ...
              return (
                ...
                >
                  ...
                  {types.map((t) => (
                    <td
                      key={t}
                      className={`py-2 px-3 text-center border-r border-gray-200 ${matrix[prod][t] ? "" : "text-gray-300"}`}
                    >
                      {matrix[prod][t] || "—"}
                    </td>
                  ))}
...
function ActivityPanel({
  ...
  const save = () => {
    ...
    if (isNew) createRecord("activities", dataToSave);
    else updateRecord("activities", dataToSave.id, dataToSave);
    onClose();
  };
  const del = async () => {
    if (confirm("Удалить активность?")) {
  ...
  const uniqueTypes = Array.from(
    new Set(allRecords.map((r) => r["Тип активности"]).filter(Boolean)),
  );
  const uniquePrograms = Array.from(
    new Set(allRecords.map((r) => r["Программа"]).filter(Boolean)),
  );
  const uniquePotoks = Array.from(
    new Set(allRecords.map((r) => r["Поток"]).filter(Boolean)),
  );
  const uniqueMentors = Array.from(
    new Set(allRecords.map((r) => r["Ментор"]).filter(Boolean)),
  );
    ...
    if (isDateField(k)) {
      return (
        <input
          type="date"
          className={className}
          value={toDateInput(val)}
          onChange={(e) => setVal(fromDateInput(e.target.value))}
        />
      );
    }
  ...
  const fields = [
    ...
    "Поток",
    "Кол-во",
    "Продолжительность факт",
    "Продолжительность план",
    "Кол-во участников",
    "source",
  ];
    ...
    <div className="w-96 border-l border-gray-200 bg-white shadow-xl flex flex-col z-10 absolute right-0 top-0 bottom-0">
      <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
        <h3 className="font-bold text-lg">
          {isNew ? "Новая активность" : "Активность"}
        </h3>
      ...
      <div className="p-4 border-t border-gray-200 bg-gray-50 flex justify-between">
        {!isNew ? (
          <button
            onClick={del}
            className="text-red-500 hover:bg-red-50 px-3 py-1 rounded text-sm flex items-center gap-1"
          >
